...
class BackTesting:
    ...
    def build_model(self):
        ...
        self.model = LSTMModel(input_size=self.X_train.shape[2], hidden_size=50, output_size=1, num_layers=1, device=self.device).to(self.device)
    ...
    def run_backtest(self, data_file_path):
        ...
        self.build_model()
        self.train_model(epochs=1, batch_size=16, learning_rate=0.001, patience=8)
        self.evaluate_model()
        ...
        self.calculate_performance_metrics(strategy)
    ...
    def calculate_performance_metrics(self, strategy):
        ...
        portfolio_series = pd.Series(
            ...
            index=pd.date_range(
                start=self.data.index[-len(strategy.portfolio_value)],
                periods=len(strategy.portfolio_value),
        ...
        hit_ratio = (strategy.winning_trades / strategy.total_trades) * 100 if strategy.total_trades > 0 else 0
        strategy_gain = (strategy.portfolio_value[-1] - strategy.initial_cash) / strategy.initial_cash * 100
