...
class BaseStrategy(ABC):
    ...
    def fetch_actual_price(self, timestamp):
        try:
            ...
            # Debug: Print the timestamp and the corresponding row in X_test_df
    ...
    def evaluate_decisions(self):
           ...
           # print(f"Timestamp: {decision['timestamp']}, Prediction: {decision['prediction']}, Actual Price: {actual_price}")
            ...
            if actual_price is not None:
                ...
                if decision["action"] == "buy":
                    ...
                    y_true_signal.append(1 if actual_price > decision["current_price"] else 0)  # Actual buy signal if price increased
                elif decision["action"] == "sell":
                    y_pred_signal.append(0)  # Predicted sell signal
                    y_true_signal.append(0 if actual_price < decision["current_price"] else 1)  # Actual sell signal if price decreased
        ...
        disp_signal = ConfusionMatrixDisplay(confusion_matrix=cm_signal, display_labels=["Buy", "Sell"])
        disp_signal.plot()
    ...
    def run_backtest(self, data_file_path):
        ...
        for i in range(len(model.X_test_df)-model.time_step):  
            ...
            future_timestamp = model.X_test_df.index[i + 1]
              # Print future_timestamp only for the first iteration
            if i == 0:
